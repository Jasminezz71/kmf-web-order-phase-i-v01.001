...
namespace ZEN.SaleAndTranfer.UI.Areas.IMPORTANDEXPORT.Controllers
{
    ...
    {
        public const string ImportAndExportTitle = "Location";
        private string v_fullPath
        {
            get
            {
                if (Session["FULL_PATH_Location"] != null)
                {
                    return (string)Session["FULL_PATH_Location"];
                }
            ...
            set
            {
                Session["FULL_PATH_Location"] = value;
            }
        }
        private string v_fileName
        {
            get
            {
                if (Session["FULL_NAME_Location"] != null)
                {
                    return (string)Session["FULL_NAME_Location"];
                }
            ...
            set
            {
                Session["FULL_NAME_Location"] = value;
            }
        }
        //
        // GET: /IMPORTANDEXPORT/Location/
        public ActionResult Index(LocationVM vm)
        {
            try
            {
        ...
        public ActionResult ExportFile(LocationVM vm)
        {
            int batchID = -1;
            ...
            try
            {
                ...
                string fileName = GetFileNameForDownload(ConfigNameEnum.LOCATION);
                ...
                LocationBC bc = new LocationBC();
                // Start batch get batchID
                batchID = batchBC.StartBatch(vm.SessionLogin.USER_NAME, "Download " + ConfigNameEnum.LOCATION.ToString(), "Download " + ConfigNameEnum.LOCATION.ToString());
        ...
        public ActionResult ImportFile(LocationVM vm)
        {
            int batchID = -1;
            BatchBC batchBC = new BatchBC();
            try
            {
                ...
                vm.SessionLogin = GetCurrentUser;
                // Start batch get batchID
                batchID = batchBC.StartBatch(vm.SessionLogin.USER_NAME, "Upload " + ConfigNameEnum.LOCATION.ToString(), "Upload " + ConfigNameEnum.LOCATION.ToString());

                LocationBC locationBC = new LocationBC();
                var tempFile = Request.Files[0];
                vm = locationBC.ValidateImportFile(tempFile, vm);
                if (vm.MessageList.Count > 0)
                {
                    batchBC.InsertBatchLog(batchID, "Import File", "" + ConfigNameEnum.LOCATION.ToString(), "Error"
                ...
                else
                {
                    ...
                    vm = locationBC.ImportFile(vm, batchID, path);
                    if (vm.MessageList.Count > 0)
                    {
