...
namespace ZEN.SaleAndTranfer.UI.Areas.IMPORTANDEXPORT.Controllers
{
    ...
    {
        public const string ImportAndExportTitle = "การเปลี่ยนหน่วยสินค้า ";
        private string v_fullPath
        {
            get
            {
                if (Session["FULL_PATH_UnitConversion"] != null)
                {
                    return (string)Session["FULL_PATH_UnitConversion"];
                }
            ...
            set
            {
                Session["FULL_PATH_UnitConversion"] = value;
            }
        }
        private string v_fileName
        {
            get
            {
                if (Session["FULL_NAME_UnitConversion"] != null)
                {
                    return (string)Session["FULL_NAME_UnitConversion"];
                }
            ...
            set
            {
                Session["FULL_NAME_UnitConversion"] = value;
            }
        }
        //
        // GET: /IMPORTANDEXPORT/UnitConversion/
        public ActionResult Index(UnitConversionVM vm)
        {
            try
            {
                InitMesssageViewBagUI();
                ViewBag.ImportAndExportTitle = ImportAndExportTitle;
                if (vm.unitConversionVM_MA != null)
                {
                    if (vm.unitConversionVM_MA.DOWNLOAD_FILE == "TRUE")
                    {
                        ViewBag.DownloadFile = "TRUE";
                    }
                }
            }
            catch (Exception ex)
        ...
        public ActionResult ExportFile(UnitConversionVM vm)
        {
            int batchID = -1;
            ...
            try
            {
                ...
                string fileName = GetFileNameForDownload(ConfigNameEnum.UNITCONVERSION);
                ...
                UnitConversionBC bc = new UnitConversionBC();
                // Start batch get batchID
                batchID = batchBC.StartBatch(vm.SessionLogin.USER_NAME, "Download " + ConfigNameEnum.UNITCONVERSION.ToString(), "Download " + ConfigNameEnum.UNITCONVERSION.ToString());
        ...
        public ActionResult ImportFile(UnitConversionVM vm)
        {
            int batchID = -1;
            BatchBC batchBC = new BatchBC();
            try
            {
                ...
                vm.SessionLogin = GetCurrentUser;
                // Start batch get batchID
                batchID = batchBC.StartBatch(vm.SessionLogin.USER_NAME, "Upload " + ConfigNameEnum.UNITCONVERSION.ToString(), "Upload " + ConfigNameEnum.UNITCONVERSION.ToString());

                UnitConversionBC unitConversionBC = new UnitConversionBC();
