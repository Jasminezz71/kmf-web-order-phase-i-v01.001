...
namespace ZEN.SaleAndTranfer.UI.Areas.IMPORTANDEXPORT.Controllers
{
    ...
    {
        public const string ImportAndExportTitle = "หน่วยสินค้า ";
        private string v_fullPath
        {
            get
            {
                if (Session["FULL_PATH_Unit"] != null)
                {
                    return (string)Session["FULL_PATH_Unit"];
                }
        ...
        private string v_fileName
        {
            get
            {
                if (Session["FULL_NAME_Unit"] != null)
                {
                    return (string)Session["FULL_NAME_Unit"];
                }
            ...
            set
            {
                Session["FULL_NAME_Unit"] = value;
            }
        }
        //
        // GET: /IMPORTANDEXPORT/Unit/
        public ActionResult Index(UnitVM vm)
        {
            try
            {
        ...
        public ActionResult ExportFile(UnitVM vm)
        {
            int batchID = -1;
            ...
            try
            {
                ...
                string fileName = GetFileNameForDownload(ConfigNameEnum.UNIT);
                ...
                UnitBC bc = new UnitBC();
                // Start batch get batchID
                batchID = batchBC.StartBatch(vm.SessionLogin.USER_NAME, "Download " + ConfigNameEnum.UNIT.ToString(), "Download " + ConfigNameEnum.UNIT.ToString());
        ...
        public ActionResult ImportFile(UnitVM vm)
        {
            int batchID = -1;
            BatchBC batchBC = new BatchBC();
            try
            {
                ...
                vm.SessionLogin = GetCurrentUser;
                // Start batch get batchID
                batchID = batchBC.StartBatch(vm.SessionLogin.USER_NAME, "Upload " + ConfigNameEnum.UNIT.ToString(), "Upload " + ConfigNameEnum.UNIT.ToString());

                UnitBC unitBC = new UnitBC();
                vm = unitBC.ValidateImportFile(Request.Files[0], vm);
                if (vm.MessageList.Count > 0)
                {
                    batchBC.InsertBatchLog(batchID, "Import File", "" + ConfigNameEnum.UNIT.ToString(), "Error"
                ...
                else
                {
                    ...
                    vm = unitBC.ImportFile(vm, batchID, path);
                    if (vm.MessageList.Count > 0)
                    {
