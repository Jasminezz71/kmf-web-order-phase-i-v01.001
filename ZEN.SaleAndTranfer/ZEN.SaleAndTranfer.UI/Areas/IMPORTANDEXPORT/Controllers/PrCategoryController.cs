...
{
    public class PrCategoryController : BaseController
    {
        ...
        public ActionResult ImportFile(PrCategoryVM vm)
        {
            ...
            try
            {
                ...
                vm.SessionLogin = GetCurrentUser;
                // Start batch get batchID
                batchID = batchBC.StartBatch(vm.SessionLogin.USER_NAME, "Upload " + ConfigNameEnum.PR_CATEGORY.ToString(), "Upload " + ConfigNameEnum.PR_CATEGORY.ToString());

                PrCategoryBC PrCategoryBC = new PrCategoryBC();
                vm = PrCategoryBC.ValidateImportFile(Request.Files[0], vm);
                if (vm.MessageList.Count > 0)
                {
                    batchBC.InsertBatchLog(batchID, "Import File", "" + ConfigNameEnum.PR_CATEGORY.ToString(), "Error"
