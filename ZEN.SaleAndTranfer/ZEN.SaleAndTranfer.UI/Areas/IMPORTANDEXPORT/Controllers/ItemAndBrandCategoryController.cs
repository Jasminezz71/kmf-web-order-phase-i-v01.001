...
{
    public class ItemAndBrandCategoryController : BaseController
    {
        public const string ImportAndExportTitle = "ความสัมพันธ์ของประเภทสินค้า กับ แบรนด์ ";
        private string v_fullPath
        {
            get
            {
                if (Session["FULL_PATH_ItemAndBrandCategory"] != null)
                {
                    return (string)Session["FULL_PATH_ItemAndBrandCategory"];
                }
            ...
            set
            {
                Session["FULL_PATH_ItemAndBrandCategory"] = value;
            }
        }
        private string v_fileName
        {
            get
            {
                if (Session["FULL_NAME_ItemAndBrandCategory"] != null)
                {
                    return (string)Session["FULL_NAME_ItemAndBrandCategory"];
                }
            ...
            set
            {
                Session["FULL_NAME_ItemAndBrandCategory"] = value;
            }
        }
        //
        // GET: /IMPORTANDEXPORT/ItemAndBrandCategory/
        public ActionResult Index(ItemAndBrandCategoryVM vm)
        {
            try
            {
        ...
        public ActionResult ExportFile(ItemAndBrandCategoryVM vm)
        {
            int batchID = -1;
            ...
            try
            {
                ...
                string fileName = GetFileNameForDownload(ConfigNameEnum.BRAND_MAP_BRAND_CATEGORY);
                ...
                ItemAndBrandCategoryBC bc = new ItemAndBrandCategoryBC();
                // Start batch get batchID
                batchID = batchBC.StartBatch(vm.SessionLogin.USER_NAME, "Download " + ConfigNameEnum.BRAND_MAP_BRAND_CATEGORY.ToString(), "Download " + ConfigNameEnum.BRAND_MAP_BRAND_CATEGORY.ToString());
        ...
        public ActionResult ImportFile(ItemAndBrandCategoryVM vm)
        {
            int batchID = -1;
            BatchBC batchBC = new BatchBC();
            try
            {
                ...
                vm.SessionLogin = GetCurrentUser;
                // Start batch get batchID
                batchID = batchBC.StartBatch(vm.SessionLogin.USER_NAME, "Upload " + ConfigNameEnum.BRAND_MAP_BRAND_CATEGORY.ToString(), "Upload " + ConfigNameEnum.BRAND_MAP_BRAND_CATEGORY.ToString());

                ItemAndBrandCategoryBC ItemAndBrandCategoryBC = new ItemAndBrandCategoryBC();
                vm = ItemAndBrandCategoryBC.ValidateImportFile(Request.Files[0], vm);
                if (vm.MessageList.Count > 0)
                {
                    batchBC.InsertBatchLog(batchID, "Import File", "" + ConfigNameEnum.BRAND_MAP_BRAND_CATEGORY.ToString(), "Error"
                ...
                else
                {
                    ...
                    vm = ItemAndBrandCategoryBC.ImportFile(vm, batchID, path);
                    if (vm.MessageList.Count > 0)
                    {
