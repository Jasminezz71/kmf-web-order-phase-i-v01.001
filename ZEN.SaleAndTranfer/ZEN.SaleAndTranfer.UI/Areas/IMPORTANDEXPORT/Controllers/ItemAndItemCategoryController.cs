...
{
    public class ItemAndItemCategoryController : BaseController
    {
        public const string ImportAndExportTitle = "ความสัมพันธ์ของประเภทสินค้า กับ สินค้า ";
        private string v_fullPath
        {
            get
            {
                if (Session["FULL_PATH_ItemAndItemCategory"] != null)
                {
                    return (string)Session["FULL_PATH_ItemAndItemCategory"];
                }
            ...
            set
            {
                Session["FULL_PATH_ItemAndItemCategory"] = value;
            }
        }
        private string v_fileName
        {
            get
            {
                if (Session["FULL_NAME_ItemAndItemCategory"] != null)
                {
                    return (string)Session["FULL_NAME_ItemAndItemCategory"];
                }
            ...
            set
            {
                Session["FULL_NAME_ItemAndItemCategory"] = value;
            }
        }
        //
        // GET: /IMPORTANDEXPORT/ItemAndItemCategory/
        public ActionResult Index(ItemAndItemCategoryVM vm)
        {
            try
            {
        ...
        public ActionResult ExportFile(ItemAndItemCategoryVM vm)
        {
            int batchID = -1;
            ...
            try
            {
                ...
                string fileName = GetFileNameForDownload(ConfigNameEnum.BRAND_MAP_ITEM_CATEGORY);
                ...
                ItemAndItemCategoryBC bc = new ItemAndItemCategoryBC();
                // Start batch get batchID
                batchID = batchBC.StartBatch(vm.SessionLogin.USER_NAME, "Download " + ConfigNameEnum.BRAND_MAP_ITEM_CATEGORY.ToString(), "Download " + ConfigNameEnum.BRAND_MAP_ITEM_CATEGORY.ToString());
        ...
        public ActionResult ImportFile(ItemAndItemCategoryVM vm)
        {
            int batchID = -1;
            BatchBC batchBC = new BatchBC();
            try
            {
                ...
                vm.SessionLogin = GetCurrentUser;
                // Start batch get batchID
                batchID = batchBC.StartBatch(vm.SessionLogin.USER_NAME, "Upload " + ConfigNameEnum.BRAND_MAP_ITEM_CATEGORY.ToString(), "Upload " + ConfigNameEnum.BRAND_MAP_ITEM_CATEGORY.ToString());

                ItemAndItemCategoryBC ItemAndItemCategoryBC = new ItemAndItemCategoryBC();
                vm = ItemAndItemCategoryBC.ValidateImportFile(Request.Files[0], vm);
                if (vm.MessageList.Count > 0)
                {
                    batchBC.InsertBatchLog(batchID, "Import File", "" + ConfigNameEnum.BRAND_MAP_ITEM_CATEGORY.ToString(), "Error"
                ...
                else
                {
                    ...
                    vm = ItemAndItemCategoryBC.ImportFile(vm, batchID, path);
                    if (vm.MessageList.Count > 0)
                    {
