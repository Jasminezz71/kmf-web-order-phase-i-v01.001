...
{
    public class ItemCategoryDC : BaseDC
    {
        public List<ItemCategorySearchResultET> ExportFile()
        {
            try
            {
                List<ItemCategorySearchResultET> result = new List<ItemCategorySearchResultET>();

                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    conn.Open();

                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_Download_WH_ITEM_CATEGORY, conn))
                    {
                        ...
                        while (reader.Read())
                        {
                            var newInstance = new ItemCategorySearchResultET();
            ...
            catch (Exception ex)
            {
                ...
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_Download_WH_ITEM_CATEGORY);
                throw ex;
            }
        }
        ...
        public List<MessageET> ValidateTempTable(int batchID, string userName) // Use Store to connect DB
        {
            try
            {
                ...
                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    ...
                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_validate_Upload_WH_ITEM_CATEGORY, conn))
                    {
                        cm.CommandType = CommandType.StoredProcedure;
                        cm.CommandTimeout = getSqlTimeout();
            ...
            {
                LogDC dcLog = new LogDC();
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_validate_Upload_WH_ITEM_CATEGORY);
                throw ex;
            }
        }
        public string ImportFile(int batchID) // Use Store to connect DB
        {
            try
            {
                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    ...
                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_Upload_WH_ITEM_CATEGORY, conn))
                    {
                        cm.CommandType = CommandType.StoredProcedure;
                        cm.CommandTimeout = getSqlTimeout();
            ...
            {
                LogDC dcLog = new LogDC();
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_Upload_WH_ITEM_CATEGORY);
                throw ex;
            }
        }
    }
