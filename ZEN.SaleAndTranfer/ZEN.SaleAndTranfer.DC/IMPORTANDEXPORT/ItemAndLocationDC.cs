...
{
    public class ItemAndLocationDC : BaseDC
    {
        public List<ItemAndLocationSearchResultET> ExportFile(string username)
        {
            try
            {
                List<ItemAndLocationSearchResultET> result = new List<ItemAndLocationSearchResultET>();

                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    conn.Open();

                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_Download_WH_MAP_ITEM, conn))
                    {
                        ...
                        while (reader.Read())
                        {
                            var newInstance = new ItemAndLocationSearchResultET();
                            ...
                            newInstance.REQUEST_TO_LOCATION_CODE = reader["REQUEST_TO_LOCATION_CODE"].ToString();
                            ...
                            newInstance.ITEM_NAME_1 = reader["ITEM_NAME_1"].ToString();
                            newInstance.ITEM_NAME_2 = reader["ITEM_NAME_2"].ToString();
            ...
            catch (Exception ex)
            {
                ...
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_Download_WH_MAP_ITEM);
                throw ex;
            }
        }
        ...
        public List<MessageET> ValidateTempTable(int batchID, string userName) // Use Store to connect DB
        {
            try
            {
                ...
                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    ...
                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_Validate_Upload_MAP_WH_ITEM, conn))
                    {
                        cm.CommandType = CommandType.StoredProcedure;
                        cm.CommandTimeout = getSqlTimeout();
            ...
            {
                LogDC dcLog = new LogDC();
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_Validate_Upload_MAP_WH_ITEM);
                throw ex;
            }
        }
        public string ImportFile(int batchID) // Use Store to connect DB
        {
            try
            {
                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    ...
                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_Upload_MAP_WH_ITEM, conn))
                    {
                        cm.CommandType = CommandType.StoredProcedure;
                        cm.CommandTimeout = getSqlTimeout();
            ...
            {
                LogDC dcLog = new LogDC();
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_Upload_MAP_WH_ITEM);
                throw ex;
            }
        }
    }
