...
{
    public class UnitDC : BaseDC
    {
        public List<UnitSearchResultET> ExportFile()
        {
            try
            {
                List<UnitSearchResultET> result = new List<UnitSearchResultET>();

                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    conn.Open();

                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_Download_UNIT, conn))
                    {
                        ...
                        while (reader.Read())
                        {
                            var newInstance = new UnitSearchResultET();

                            newInstance.UNIT_CODE = reader["UNIT_CODE"].ToString();
                            newInstance.NAV_UNIT = reader["NAV_UNIT"].ToString();
            ...
            catch (Exception ex)
            {
                ...
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_Download_UNIT);
                throw ex;
            }
        }
        ...
        public List<MessageET> ValidateTempTable(int batchID, string userName) // Use Store to connect DB
        {
            try
            {
                ...
                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    ...
                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_validate_Upload_NON_NAV_UNIT, conn))
                    {
                        cm.CommandType = CommandType.StoredProcedure;
                        cm.CommandTimeout = getSqlTimeout();
            ...
            {
                LogDC dcLog = new LogDC();
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_validate_Upload_NON_NAV_UNIT);
                throw ex;
            }
        }
        public string ImportFile(int batchID) // Use Store to connect DB
        {
            try
            {
                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    ...
                    #region -- header --
                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_Upload_NON_NAV_UNIT, conn))
                    {
                        cm.CommandType = CommandType.StoredProcedure;
                        cm.CommandTimeout = getSqlTimeout();
            ...
            {
                LogDC dcLog = new LogDC();
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_Upload_NON_NAV_UNIT);
                throw ex;
            }
        }
    }
