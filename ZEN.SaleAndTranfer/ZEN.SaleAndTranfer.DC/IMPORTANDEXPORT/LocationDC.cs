...
{
    public class LocationDC : BaseDC
    {
        public List<LocationSearchResultET> ExportFile()
        {
            try
            {
                List<LocationSearchResultET> result = new List<LocationSearchResultET>();

                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    conn.Open();

                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_Download_LOCATION, conn))
                    {
                        ...
                        while (reader.Read())
                        {
                            var newInstance = new LocationSearchResultET();

                            newInstance.LOCATION_CODE = reader["LOCATION_CODE"].ToString();
                            newInstance.LOCATION_NAME = reader["LOCATION_NAME"].ToString();
            ...
            catch (Exception ex)
            {
                ...
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_Download_LOCATION);
                throw ex;
            }
        }

        public int ImportTempTable(DataTable d1) // Bluk Insert not use Database
        {
            try
            {
                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
        ...
        public List<MessageET> ValidateTempTable(int batchID, string userName) // Use Database
        {
            try
            {
                ...
                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    ...
                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_validate_Upload_LOCATION, conn))
                    {
                        cm.CommandType = CommandType.StoredProcedure;
                        cm.CommandTimeout = getSqlTimeout();
            ...
            {
                LogDC dcLog = new LogDC();
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_validate_Upload_LOCATION);
                throw ex;
            }
        }
        public string ImportFile(int batchID) // Use Database
        {
            try
            {
                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    ...
                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_Upload_LOCATION, conn))
                    {
                        cm.CommandType = CommandType.StoredProcedure;
                        cm.CommandTimeout = getSqlTimeout();
            ...
            {
                LogDC dcLog = new LogDC();
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_Upload_LOCATION);
                throw ex;
            }
        }
    }
