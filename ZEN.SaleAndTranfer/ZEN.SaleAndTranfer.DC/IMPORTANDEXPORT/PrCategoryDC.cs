...
{
    public class PrCategoryDC : BaseDC
    {
        public List<PrCategorySearchResultET> ExportFile()
        {
            try
            {
                List<PrCategorySearchResultET> result = new List<PrCategorySearchResultET>();

                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    conn.Open();

                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_Download_BRAND_CATEGORY, conn))
                    {
                        ...
                        while (reader.Read())
                        {
                            var newInstance = new PrCategorySearchResultET();
            ...
            catch (Exception ex)
            {
                ...
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_Download_BRAND_CATEGORY);
                throw ex;
            }
        }
        ...
        public List<MessageET> ValidateTempTable(int batchID, string userName) // Use Store to connect DB
        {
            try
            {
                ...
                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    ...
                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_validate_Upload_PR_CATEGORY, conn))
                    {
                        cm.CommandType = CommandType.StoredProcedure;
                        cm.CommandTimeout = getSqlTimeout();
            ...
            {
                LogDC dcLog = new LogDC();
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_validate_Upload_PR_CATEGORY);
                throw ex;
            }
        }
        public string ImportFile(int batchID) // Use Store to connect DB
        {
            try
            {
                using (var conn = new SqlConnection(ConfigConst.CONN_STR_DEF))
                {
                    ...
                    using (var cm = new SqlCommand(StoreProcConst.USP_ST_Upload_PR_CATEGORY, conn))
                    {
                        cm.CommandType = CommandType.StoredProcedure;
                        cm.CommandTimeout = getSqlTimeout();
            ...
            {
                LogDC dcLog = new LogDC();
                dcLog.InsertLogDC(ex, "dummySession", StoreProcConst.USP_ST_Upload_PR_CATEGORY);
                throw ex;
            }
        }
    }
